...
int main(int argc, char **argv) {
  ...
  if (sources.empty()) {
    fprintf(
        stderr,
        "*** Provided helmkf file contains no SOURCES variable. Stop. ***\n");
    return 1;
  }
  ...
  if (include.empty()) {
    include = fs::canonical(fs::path(helc).parent_path() / "../include")
                  .generic_string();
  }
  if (lib.empty()) {
    lib =
        fs::canonical(fs::path(helc).parent_path() / "../lib").generic_string();
  }
  fprintf(makefile, "\nHINCLUDE = %s\nHLIB = %s\n", include.c_str(),
          lib.c_str());
