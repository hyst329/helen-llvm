...
map<ErrorType, string> Error::errorMessages = {
    ...
    {ErrorType::UndeclaredFunction,
     "Undeclared function: %1% (mangled name: %2%)"},
    {ErrorType::WrongArgumentType, "Wrong argument type: %1%"},
    {ErrorType::WrongArgumentNumber,
     "Wrong argument type: %1% while %2% required"},
    ...
    {ErrorType::ZeroIndexError,
     "Array index must not be zero (it's not C++, so indices do start from 1)"},
    {ErrorType::UnknownStyle,
     "Unknown linkage style ('C' and 'Helen' are currently allowed)"},
    {ErrorType::TypeRedefined, "Function %1% is redefined"},
    {ErrorType::UndeclaredType, "Undeclared type: %1%"},
    {ErrorType::NonObjectType,
     "Type of the variable is probably non-object (i.e. primitive)"},
    {ErrorType::UncastableTypes, "Value cannot be casted to type"},
    {ErrorType::InvalidShiftbyUse, "Invalid use of SHIFTBY instruction"},
    {ErrorType::InterfaceInheritedFromType,
     "Interface cannot inherit from regular type"},
    {ErrorType::InvalidInstantiation,
     "Invalid instantiation: %1% is not generic"},
};
